...
**Answer**It contains a table that shows the two digit NAICS sector, and what category it describes. Since it is a two digit table, the descriptions are very broad (e.g. utilities or information). 
...
**Answer**The source code for the table looks like a long list of HTML code that is encompassed in a <table> tag. The list contains each piece of information in the table in each row.
...
**Answer**The outpit is a list of html lines in the document. 
...
**Answer**It shows the list in a format with name, type, and value, as well as a line that describes the attributes.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**By doing this, I see a section of HTML code where if I click the area with the <table> bracket, I highlight the table section on the website.
...
**Answer** This gathers all of the data from the HTML tables on the webstie.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**The naics_industry looks like it has taken the tables from the previous lines of code and placed the tibbles there. If you click the arrows, you can see the column titles and some example values in each row.
...
**Answer**This opens up the table in a new tab and allows you to see what is in each one.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**This one shows us the table that we were looking at on the website with teh NAICS sector and description. 
...
**Task** Run the following code to keep only the table we want. Briefly describe the output that appears below the code block.
**Answer** This data collapses the tables of data from the html, and keeps only the data from the second table (which is what we need) by using the index 2. It then displays this table as it stores it in naics_industry as a data frame.
...
**Answer**This code changes the column titles to be lower case, and then cuts the last row (which contains the source and URL) from the data. 
...
**Answer**The output shows a table with a column that list the two code sectors, and a column with the description of that sector.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**There are multiple tables on the web page that are divided by secotr with the amount of people in each sector in different months.
...
The table shows that for the mining sector, it was 538.7  -- or 538,700 -- in November 2021.  That's the value we want to ingest in R. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**This code takes the url and reads the html from it and then spits out the results, which is a
...
**Answer**Instead of calling html_table, calling html_element to call one specific tag. Then, the code uses the id from the table as seen on the HTML code on the website to plug that into the employment_info element. This result shows just this HTML content from the table we want. 
...
**Answer**This code now uses the html_table() function to take the html code from the table we want from the webpage and convert it into a dataframe in R.
...
**Answer** This cleans the data so we only get the information we are looking for by extracting the information from the row we needed.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**This web page has the same table with mining information.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**This web page has the same table with construction information.
...
**Answer**This output prints the values of each of the sectors, as indicated in the print statements.
```{r}
...
**Answer**This creates a list of all the different sectors that are looking for data.
```{r}
...
**Answer**This prints the name of the sector for each sector in the list.
```{r}
...
**Answer**This list shows an error because sector_list doe snot exist. You have to refer to the correct list of the for loop to be able to run. 
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**This task does the same thing as task 16, but shows how you can put any name in the spot that x is in, as long as you use the same name throughout the for loop.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**This code gets an error that says the object 'sector_name' doesn't exist, which makes sense because we refer to it as x when we are declaring the loop, so we'd need to use x again for the loop to recognize what we are referencing. 
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**This loop iterates over a range of numbers from 1 to 10 and prints each number as it runs through the loop.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**This output shows the naics_industry dataframe with each sector and description. There are 20 rows and 2 columns.
...
**Answer**This output shows us the number of rows in the naics_industry dataframe, 20.
...
**Answer** This code loops through all industries in the list and figure out the number of rows that are in the data frame.
...
**Answer**This code loads the table from earlier that we learned how to get using html_element() and html_table(). It shows us the number of employees in this sector there were in november 2021. 
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**This code creates the dataframe from the last chapter that shows us the naics sector and the name of each industry it refers to. It was taken from an HTML table from the BLS website.
```{r}
# Define url of page we want to scrape
...
**Answer**This code updates the naics_indsutry table to include a column with the url that matches the sector that it applies to.
...
**Answer**This code creates a new column with the xpath ID that matches the id that we will need to call from each URL to get the data from the HTML table. Right now, it just adds those xpath ids to a new column in the naics_industry dataframe.
...
**Answer**This code removes the row that included Public Administration as the description from the naics_industry data frame. 
...
**Answer**This code runs through all of the rows in the naics_industry dataframe and saves each row to its own data frame and prints it out. The output is all 19 rows printed out.
...
**Answer**This output shows us what we are working with the for loop we've built so far. For each row in the naics_industry data frame, it prints out the row number, the URL: that corresponds to it, and the xpath that corresponds to it by drawing information from the naics_industry data frame.
```{r}
...
**Answer**This data printed out 19 dataframes, one for each row in the naics_industry dataframe. Each dataframe that printed pulled the html data from the website according to the url and xpath, and then put that each piece of information into a table for each row.
...
**Answer**This code extracts the value we needed from each html table from the previous code and then stores it in a dataframe called employment_info, and then prints the data from each of these dataframes. There are 19, one for each row in the naics_industry dataframe. 
...
**Answer**This output shows the same dataframes as the last task, but adds on the industry name to each row so that we know which employment numbers correspond to which naics industry.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**This output shows the cleaned data from the previous task that takes each data frame and renames the month complumn to be more specific by including "employee" in the name. It also selects only that information that is needed for the final dataframe, which is the sector number, the description, and the number of employees.
...
**Answer**This ouput combines the data from each individual dataframe that we've been refining up until this point, and placing it all together in an empty dataframe titled "employemnet_by_sector_all." It includes all of the data from the individual dataframes that each had one row. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**This web page shows us that the value for the mining sector is 536.9, which matches what is put in the employment table we created.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**This webpage shows us that for the "Agriculture, Forestry, Fishing and Hunting" table, it only has the unemployment rate, and it does not include data for number of employeed people in that industry.
...
**Answer**This output removes the value for number of employees from the agriculture sector row, and leaves it in the table as N/A. 
