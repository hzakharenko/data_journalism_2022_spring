...
**Answer**The data includes vaccination data from different counties in MD, including first dose, second dose, single dose, etc. from December 2020 through March 2022. There seem to be a lot of NA values in the county column.
...
**Answer** The number of days that each county has records for is slightly different, and there are 284 days of county values marked as NA and 448 days of county values marked as Unknown.
...
**Answer**This data shows all of the data where county is marked as NA. It is unclear whether the county data represents the county that the shot was delivered in, the residence of the person getting a vaccine, the county government administering the data, etc. So, these NA values could be mistakes or they could mean something else (like private doses, etc.)
...
**Answer**The number of vaccinations seem to be decreasing, and none of them are among high ranked days (which means that less people are gettign vaccinated).
...
**Answer**This chart shows the number of vaccinations over time. The trend is a spike shortly after vaccines were introduced, followed by a quick dip, and then a bigger spike followed by a quick but relatively steady decline.
...
**Answer**This chart shows the number of cases in each Maryland county based on date
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**This chart shows the same data as the preivous chart did, but scales it by log10 so that the line that shows up for each county looks a little different, but better represents the increase in cases.
...
**Answer**This map shows the number of cases in each Maryland county on the most recent data based on the day that the code is run (in this case it shows 4/18 when it is run on 4/19).
