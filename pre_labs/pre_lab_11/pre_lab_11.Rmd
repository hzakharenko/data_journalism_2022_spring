...
**Answer** This image shows the different formats of geographic data and how shapefiles involve multiple files.
...
**Answer**This data loads the hospitals dataframe into R, with columns that look similar to what we normally see in our dataframes, and some more unique columns like geometry. 
...
**Answer**This code reads in the Maryland hospitals into a dataframe md_hospitals.
...
general medical and surgical hospitals, chronic care, psychiatric, military, special. etc.

### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This count groups the Maryland hospital by type and counts how many are in each type. It then displays this count from highest to lowest.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This filters the data to only include Maryland hospitals with the type "general acute care" and stores it to the dataframe md_hospitals.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This is a scatterplot that looks pretty similar to the shape of Maryland. In this scatterplot, each point is a hospital and it is graphes based on location.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code loads the package 'tigris' which is used to get data from the U.S. Census.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code uses the county funciton to pull a shapefile of US counties, stores it in the dataframe counties and displays some basic information of this dataframe.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code filters the US counties and saves the Maryland counties to the dataframe md_counties.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code plots the Maryland counties shapes using ggplot, which effectively creates a map of Maryland with county lines.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code graphs the two maps we've done in previous steps and layers them on top of each others so that we can see where the Maryland hospitals are on the map of Maryland counties.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code gives the map more meaning by changing the color of the dots relative to the number of beds that the hospital has. It also makes the county map background white so it is easier to see these dots.
...
**Answer**This code loads all the packages we need.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code groups the maryland hospitals by county, counts the total number of beds at each hospital in the county, and then arranges this from biggest to smallest number of beds in each county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code pulls the acs data (population data) for each county in Maryland.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This codes is the same as the previous task, but uses the geometry=TRUE line to add the geometry column to the dataframe so we can use this data to make a map as well.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code shows us both the dataframes that we are working with so that we can figure out how to best combine these dataframes.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code cleans up the Maryland county population dataframe so that it matches the Maryland beds by county dataframe so that it will make it easier for us to combine them.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code combines the two dataframes together and saves it to a new dataframe, md_beds_per_100k
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code combines the dataframes again and replaces all the N/A values for hospitals with 0, which will make it possible to calculate the number of beds per 100k. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code does the same as the previous 2 tasks but now calculates the beds per 100k for each county in a new column in the combined dataframe, and arranges it from biggest to smallest.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer**This code maps the data that we calculated, showing the number of beds per 100k people in each Maryland county, and using fill so that we can see which counties have more/less beds.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This code adds a theme to the previous map and then changes the scale so that it is easier to see the color differentiation between different levels of number of beds.
...
**Answer**This code takes the map from the previous dataset and changes the division in between the groups for each number of beds to be on a logarithmic scale, which allows us to see more detailed differences between all the counties. Using this, we can see that the majority of beds are in Baltimore area, near the Eastern shore and in Western Maryland. In comparison, Southern Maryland has a very small number of beds per 100k.
