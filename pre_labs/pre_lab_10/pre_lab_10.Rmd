...
https://www.datawrapper.de/_/NFslq/ 
...
This code creates a dataframe with the number of loans in each Maryland county.
...
**Answer** Describe what the code below is doing and the output.
This code gets rid of extraneous records that are not MD counties by only keeping data with the count being above 6 (because we know that's all the counties that aren't in Maryland).
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
This code creates the new column, fips_code that combines the state code and county code to be a 5-digit FIPS code.
```{r}
...
In this data set, I see the md_counties data set joined with the md_flips data set to show the count of loans in each county along with the FIPS code for each county.
...
Doing this changes the map from showing the whole U.S. to only showing Maryland and Maryland counties.
...
**Answer** Describe the changes from the previous map
This map has a color legend that shows the number of loans in each county based on the range of the data set and how it relates to the color scheme of the map. It also has a descriptive headline and chatter. The hover feature is fixed to be more descriptive to show the amount of loans each county received when it is highlighted.
...
https://www.datawrapper.de/_/19j3E/
