...
**Answer**This code produces a dataframe has the state name, who won it, the percentage they voted for trump, and the percentage of businesses that received PPP loans.
```{r}
...
**Answer**This plot shows the relationship between the percentage of people voting for Trump and the percentage of businesses that received PPP loans in each state.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer**This plot shows the relationship between the percentage of people that voted for Trump and the loan amount per employee for each state.
```{r}
...
**Answer**This code shows us the correlation between the two variables from the previous graphs and the percentage of people who voted for Trump.
```{r}
reveal_data %>%
...
**Answer**This model runs a Pearson's correlation test shows the same correlation we calculated above. It also displays a t value and p value. The p-value is very low, which means it is statistically significant, and therefore very unlikely that it this correlation happened by chance.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**This model performs the same test as the previous task but with the other variable (ppe amount per employee). In this case, the p-value is pretty high (above 0.05), which means it is not statistically significant, and that it is more likely that this outcome happened by chance. 
```{r}
...
**Answer**This table groups the original data by who won the vote in 2016 (Trump or Clinton), and then calculates the average ppp loan amount per employee and the average % of businesses that received ppp loans for each group.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer**This code runs a t-test that shows us the t-value, p-value, confidence interval and means for both groups (that was also calculated in the previous task). Since the t-value is close to 0 and the p-value is a lot more than 0.05, it means that there is little evidence that the difference in means is statistically significant. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer**This code runs another t-test for the other variable (% of businesses that received ppp loans). In this test, the p-value is a lot lower (less than 0.05) and the t-value is larger. The results of this test mean that the difference between the means might be statistically significant.
```{r}
