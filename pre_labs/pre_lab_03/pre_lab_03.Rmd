...
#best for data with a huge amount of rows (e.g. 11 million) because read_csv will check every row, which can take a while
```
...
**Answer** amount field is <dbl>(a number), name field is <chr>(a character)and ID is <dbl> (a number).
```{r}
...
**Answer**: There are 192,959	loans where the franchise_name is NA. There are 275	loans where the franchise_name is Subway.
...
**Answer**: 135,073	loans had forvgiveness_amount of NA.
...
**Answer**: 135,073	have a forgiveness_date of NA, which is the same number of loans as forgiveness_amount.
...
ppp_maryland_loans %>%
  ...
  #can put ! in front of is.na to show what is NOT NA (e.g. !is.na(forgiveness_date))
  arrange(desc(count))
```
...
**Answer**: 135,073	which is the same number of loans as if the two questions were asked separately in the codeblocks above. Based on this, it means that any loan that has not been forgiven is correct because a loan should not have one or the other, it should have both or neither (it can't have a forgiveness date or a forgiveness amount if it has not actually been forgiven).
...
**Answer**There are no loans recorded that were approved in October 2022, because there was a first draw that happened in 2020 and a second draw that happened in 2021, which explains the gap in months.
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for 10,000,000 dollars	and the smallest loan is for 6 dollars. The min amount is suspicious to me because I find it hard to believe that a business would go through a whole loan application to ask for 6 dollars.
```{r}
...
**Answer**All of the various fields do align with eachother, which means it is probably not a typo in the data entry.
...
**Answer** The amount field is a <chr> datatype (a character)
```{r}
...
**Answer** 1) The column headers are inconsistently styled, where the first column has a 1_id and the name column is in all caps, the street address column doesn't have an underscore, etc. 2) The amount column is stored as a character, not a number 3) The city Arnold is spelled a couple of different ways in the column
...
**Answer**This function changes the name of the column "1_id" to "x1_id"
...
**Answer**It says error in h(), problem with the summarise() column in total_amount because the type of the column is a character, so it cannot take the sum of it
...
**Answer**The businesses name is STUART JONES LLC
...
**Answer** 2 mispellings remain after running this code
...
**Answer**1 mispelling remains after running this code.
...
**Answer**0 mispellings of Arnold remain after running this code
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
